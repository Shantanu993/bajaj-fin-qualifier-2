...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigate to form page with user details
      router.push(`/form?rollNumber=${encodeURIComponent(rollNumber)}&name=${encodeURIComponent(name)}`)
    } catch (err) {
      console.error(err)
    ...
    <div className="bg-white shadow-lg rounded-xl p-6">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-gray-800 text-white py-2 px-4 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors disabled:opacity-70"
